  ...
  async function posTab() {
    ...
    return `
      ...
      <div class="grid-stats">
        ...
        <div class="stat good"><div class="label">Today's Profit</div><div class="value">${fmt(s.totalProfit)}</div></div>
  ...
  function itemsTab() {
    ...
    return `
      <div class="pagehead"><div><h2>Inventory</h2><p>Manage what you sell</p></div>
        <button class="primary" id="btnAddItem">+ Add Item</button></div>
      ${onboarding}
  ...
  async function creditTab() {
    ...
    const rows = currentDebts.map(d => `
      <tr class="${d.balance >= 500 ? 'debt-high' : ''}">
        ...
        <td>${fmt(d.balance)}</td>
        <td>
          <button class="ghost small" data-pay="${d.id}">Record Payment</button>
          <button class="ghost small" data-adddebt="${d.id}">+ Add Credit</button>
          <button class="warn small" data-delp="${d.id}">Delete</button>
        </td>
      </tr>`).join('');
    return `
  ...
  async function reportsTab() {
    ...
    return `
      ...
      <div class="grid-stats">
        ...
        <div class="stat good"><div class="label">Today's Profit</div><div class="value">${fmt(sToday.totalProfit)}</div></div>
  ...
  async function settingsTab() {
    ...
    return `
      ...
      <div class="card">
        ...
        <p class="muted">All your data is stored on this device:</p>
      ...
      <div class="card">
        ...
        <p class="muted">Import your existing chaico_data.db backup (from the desktop app or an older phone backup), or export the current data as a .db file to keep safe.</p>
        <div class="cart-actions">
